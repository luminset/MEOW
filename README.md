...
- 如果已有旧版本配置文件，程序会先读取其中已经启用的配置项，再按当前版本模板重新生成完整配置文件，并把旧配置值回填到对应分组。
- 程序不再把新配置项追加到旧配置文件末尾，历史版本反复追加造成的重复配置分组会在重建时被清理。
...
- `keep`：保连接模式。默认判定逻辑不变；当请求需要走上游代理但所有上游代理连接失败，或上游代理真实请求返回 `parentFallbackStatus` 中的状态码时，程序会再尝试直接连接目标网站。
- `cow`：COW 模式。未命中黑名单或显式代理规则时默认直连；如果直连失败且配置了上游代理，则快速切换为通过上游代理尝试连接。

### 状态码回退

状态码回退分为两个方向。程序会强制校验：同一个状态码不能同时出现在“直连转代理”和“代理转直连”两侧，否则启动时报错退出。
...
每个请求最多只会在直连和二级代理之间切换一次，避免异常状态码导致循环重试。`parentProbeFailStatus` 仅用于 `loadBalance = latency` 的周期探测，不参与真实请求的回退判断。比如 `httpErrorCode = 503` 与默认 `parentFallbackStatus = 502,503,504` 会冲突，程序会要求你删除其中一侧配置。
